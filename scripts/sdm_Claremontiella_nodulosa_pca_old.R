...
x <- as.data.frame(sp, xy = T) %>% 
  ...
  vect(
    ., 
    # geom = c("x", "y"), 
    crs = crs(pa$`Claremontiella nodulosa`)#, 
    # extent = ext(pa$`Claremontiella nodulosa`)
  )
ext(x) <- terra::ext(pa$`Claremontiella nodulosa`)
x <- pa$`Claremontiella nodulosa` %>% 
  as.data.frame(xy = T) %>% 
  filter(individualCount > 0) %>% 
  select(lon = x, lat = y)
ppca <- ggplot() + 
  geom_raster(
    data = as.data.frame(climosaic_pca$PC1, xy = T), 
    aes(x = x, y = y, fill = PC1)
  ) + 
  geom_sf(data = spp_sf)
clm01 <- terra::extract(
  climosaic_pca, spp_sf, xy = T
)
ppno <- ggplot() + 
  geom_tile(
    data = as.data.frame(climosaic$chla, xy = T), 
    aes(x = x, y = y, fill = chla)
  ) + 
  geom_sf(data = spp_sf)
clm01 <- terra::extract(
  climosaic, spp_sf, xy = T
)
clm02 <- terra::extract(
  climosaic_pca, x, xy = T
)
my_cells <- terra::cells(pa$`Claremontiella nodulosa`, 1)$individualCount
lapp(c(climosaic_pca, pa$`Claremontiella nodulosa`*1000)
my_cells <- terra::cells(pa$`Claremontiella nodulosa`, 1)$individualCount
clm_cells <- terra::extract(
  climosaic_pca, my_cells, xy = T
)
ggplot() + 
  geom_raster(
  ...
  geom_sf(data = maps$GLP, alpha = 0.5, fill = "lightgreen") + 
  geom_point(data = clm_cells, aes(x, y, col = is.na(PC1))
  )
table(is.na(clm01[, 2]))
svx <- clm01 %>% 
  filter(is.na(PC1)) %>%
  select(x, y) %>% 
  as_tibble() %>% 
  vect(., geom = c("x", "y"), crs = crs(climosaic_pca))
svy <- as.data.frame(climosaic_pca, xy = T) %>% 
  select(x, y) %>% 
  as_tibble() %>% 
  vect(., geom = c("x", "y"), crs = crs(climosaic_pca))
ngb <- nearest(x = svx, y = svy)
ngb_val <- ngb %>% values()
# changer les stations par
# leurs nearest, utiliser les climatologies pca
M <- spp %>% cbind(clm)
