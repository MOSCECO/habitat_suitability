...
# nom du modèle
vec_name_model <- c("rasterized", "pca", "nearest", "basic")
...
# ggplot() +
#   geom_tile(
#     data = as.data.frame(climosaic_pca, xy = T), 
#     aes(x, y, fill = PC1)
#   ) +
...
clm <- terra::extract(climosaic_pca, spp_sf)
...
m_extract <- stations_nearest$ANT %>% 
  ...
  st_drop_geometry() %>% 
  as.matrix()
spe_env <- terra::cellFromXY(climosaic_pca, m_extract)
spe_cell <- cellFromXY(
  climosaic_pca,
  spp[, c("x", "y")] %>% 
    filter(!duplicated(.))
