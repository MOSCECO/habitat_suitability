...
climatologies_islands <- lapply(
  ...
  function(isl) {
    ...
    climatologies_varenv <- foreach(
      ...
      nc <- path %>% 
        list.files(., full.names = T, pattern = paste0("*", varenv, ".nc")) %>%
        tidync()
      br <- path %>% 
        list.files(., full.names = T, pattern = paste0("*", varenv, ".nc")) %>%
        brick()
      ...
      climatology_mini <- if (
        ...
        print("Minimum")
        calc(my_raster_slice, min, na.rm = T)
      }
      ...
      climatology_maxi <- if (
        ...
        print("Maximum")
        calc(my_raster_slice, max, na.rm = T)
      }
