              ...
              tryCatch(
                {
                  ...
                  my_date_up <- (my_date + wdw) %>% as.POSIXct() %>% as.numeric()
                  my_date_dw <- (my_date - wdw) %>% as.POSIXct() %>% as.numeric()
                  ...
                  # la station
                  r    <- lapply(tb, function(x) rasterFromXYZ(x[, c(2, 3, 1)]))
  ...
  function(isl) {
    ...
    min_tbs <- lapply(
      ...
      function(varenv) {
        ...
        return(v %>% dplyr::select(c(1, my_labels_pos)))
      }
    )
