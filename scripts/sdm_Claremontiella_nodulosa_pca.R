...
# nom du modèle
vec_name_model <- c("rasterized", "pca")
...
clma <- terra::extract(climosaic_rast, spa_sf)
M <- spp %>% cbind(clm)

# données manquantes dans la matrice occurrence/environnement 
# profondeurs : problème avec le gebco
M$depth[is.na(M$depth)] <- -1
# salinité : associer la salinité à 9m pour les stations manquantes
M$so[is.na(M$so)] <- M$so9[is.na(M$so)]
M <- M %>% select(-so9)
m <- M[which(names(M) == "chla"):ncol(M)]
(vif_excluded <- usdm::vifstep(m)@excluded)
varenv_subset <- names(m)[-which(names(m) %in% vif_excluded)]
m <- m %>% select(all_of(varenv_subset))

# grille spatiale pour le sous-échantillon de variables environnementales
# non colinéaires (méthode du vif)
clim_sub <- terra::subset(climosaic_rast, varenv_subset)
# mclm_sub <- terra::crds(clim_sub) %>% 
#   cbind(terra::values(clim_sub, na.rm = T))

# ACP centrée normée ----
# res_pca <- prcomp(mclm_sub[, -c(1:2)], center = T, scale. = T)
res_pca <- prcomp(m, center = T, scale. = T)
data_evplot <- res_pca$sdev^2  
x11()
evplot(data_evplot)
fviz_pca_biplot(res_pca)
fviz_contrib(res_pca, "var", axes = c(1))
fviz_contrib(res_pca, "var", axes = c(2))
fviz_contrib(res_pca, "var", axes = c(3))
fviz_contrib(res_pca, "var", axes = c(1, 2))

# création d'une climatologie à partir des données des axes de la PCA
# mclm <-  mclm_sub[, c(1:2)] %>% 
#   cbind(res_pca$x[, c(1,2)]) %>% 
#   as.data.frame(xy = T)
# terra::values(clim_sub, na.rm = T)[!is.na(terra::values(clim_sub))] <- mclm$PC1
# terra::values(clim_sub) %>% apply(2, \(x) table(is.na(x)))
# r1 <- clim_sub$chla
# values(r1) <- ifelse(values(is.na(r1)), 0, 1)
# r2 <- clim_sub$depth
# values(r2) <- ifelse(values(is.na(r2)), 0, 1)
# r3 <- clim_sub$sbt
# values(r3) <- ifelse(values(is.na(r3)), 0, 1)
# r4 <- clim_sub$so
# values(r4) <- ifelse(values(is.na(r4)), 0, 1)
# r5 <- rast(list(r1, r2, r3, r4))
# plot(Reduce(`+`, list(r1, r2, r3, r4)))
...
spec_data <- BIOMOD_FormatingData( 
  ...
  expl.var       = res_pca$x[, 1:3] %>% as.matrix(), 
  resp.xy        = spec_sub[, c("x", "y")], 
  dir.name       = path_models, 
  resp.name      = gsub(" ", ".", paste(bn, vec_name_model, "ensemble")), 
  filter.raster  = TRUE
)
...
spec_models <- BIOMOD_Modeling(
  ...
  bm.format       = spec_data,
  modeling.id     = paste(
    gsub(" ", ".", bn), vec_name_model, "ensemble", sep = "."
  ),
...
path_eval <- here(
  "data", "analysis", "models", gsub(" ", ".", bn) %>% 
    paste(c(vec_name_model, "ensemble"), collapse = "."), "eval"
)
makeMyDir(path_eval)
file_name <- gsub(" ", ".", bn) %>% 
...
path_EM <- here(
  "data", "analysis", "models", gsub(" ", ".", bn) %>% 
    paste(c(vec_name_model, "ensemble"), collapse = "."), "ensemble"
)
makeMyDir(path_EM)
  ...
  \(EMalg, col_optn) {
    ...
    ps <- mapply(
      \(tb, nisl) {
        ...
        file_name <- gsub(" ", ".", bn) %>% 
          paste("ensemble", vec_name_model, sep = ".") %>% 
